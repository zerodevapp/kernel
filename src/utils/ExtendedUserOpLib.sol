...
library ExtendedUserOpLib {
    ...
    function checkUserOpOffset(UserOperation calldata userOp) internal pure returns(bool success) {
        ...
        assembly {
            if eq(add(initCode.offset, mul(div(add(initCode.length,63),32),0x20)), cd.offset) {
                success := 1
            }
            if and(eq(add(cd.offset, mul(div(add(cd.length,63),32),0x20)), paymasterAndData.offset), success) {
                success := 1
            }
            if and(eq(add(paymasterAndData.offset, mul(div(add(paymasterAndData.length,63),32),0x20)), sig.offset), success) {
                success := 1
            }
        }
    }

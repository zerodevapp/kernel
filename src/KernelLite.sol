...
abstract contract KernelLite is EIP712, Compatibility, KernelStorage {
    ...
    function isValidSignature(bytes32 hash, bytes calldata signature) external view returns (bytes4) {
        uint256 validationData = _validateSignature(hash, signature);
    ...
    function _validateUserOp(UserOperation calldata _op, bytes32 _opHash, uint256 _missingFunds) internal virtual returns(uint256);

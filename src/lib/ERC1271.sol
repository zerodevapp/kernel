        ...
        returns (bool result)
    {
        ...
        if (t != uint256(0)) {
            ...
            assembly {
                ...
                mstore(add(t, 0x40), keccak256(add(name, 0x20), mload(name)))
                mstore(add(t, 0x60), keccak256(add(version, 0x20), mload(version)))
                mstore(add(t, 0x80), shr(96, shl(96, verifyingContract)))
                mstore(add(t, 0xa0), salt)
                mstore(0x40, add(t, 0xc0)) // Allocate the memory.
            }
        }
        /// @solidity memory-safe-assembly
        assembly {
            ...
            for {} 1 {} {
                ...
                mstore(p, " contents,string name,string") // Store the rest of the encoding.
                mstore(add(p, 0x1c), " version,address verifyingContra")
                mstore(add(p, 0x3c), "ct,bytes32 salt)")
                p := add(p, 0x4c)
                calldatacopy(p, add(o, 0x40), c) // Copy `contentsType`.
                // Fill in the missing fields of the `TypedDataSign`.
                calldatacopy(t, o, 0x40) // Copy the `contents` struct hash to `add(t, 0x20)`.
                ...
                // The "\x19\x01" prefix is already at 0x00.
                // `APP_DOMAIN_SEPARATOR` is already at 0x20.
                mstore(0x40, keccak256(t, 0xc0)) // `hashStruct(typedDataSign)`.
                // Compute the final hash, corrupted if `contentsName` is invalid.
                hash := keccak256(0x1e, add(0x42, and(1, d)))
                signature.length := sub(signature.length, l) // Truncate the signature.
                break

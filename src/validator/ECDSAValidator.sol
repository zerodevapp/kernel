...
contract ECDSAValidator is IKernelValidator {
    ...
    function enable(bytes calldata _data) external override {
        ...
        address oldOwner = ecdsaValidatorStorage[owner].owner;
        ecdsaValidatorStorage[owner].owner = owner;
        emit OwnerChanged(oldOwner, owner);
    }

    function validateUserOp(UserOperation calldata _userOp, bytes32 _userOpHash, uint256) external view override returns(uint256 validationData) {

...
contract Staker is Ownable, EIP712 {
    ...
    function deployWithFactory(address factory, bytes calldata createData)
        external
        payable
        returns (address)
    {
    ...
    function approveFactoryWithSignature(address _factory, bool approval, bytes calldata signature) external payable onlyOwner {
        // struct :
        // {
        //   factory: address,
        //   approval: bool,
        ...
        bytes32 digest = _hashTypedDataSansChainId(
            keccak256(
                abi.encode(
                    keccak256("ApproveFactory(address factory,bool approval)"),
                    _factory,
                    approval
                )
            )
